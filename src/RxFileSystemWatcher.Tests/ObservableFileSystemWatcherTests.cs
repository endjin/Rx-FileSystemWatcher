namespace Tests
{
	...
	public class ObservableFileSystemWatcherTests : FileIntegrationTestsBase
	{
		...
		{
			using var watcher = new ObservableFileSystemWatcher(c => { c.Path = this.TempPath; });
			var firstChanged = watcher.Changed.FirstAsync().ToTask();
			watcher.Start();

			await File.WriteAllTextAsync(Path.Combine(this.TempPath, "Changed.Txt"), "foo");
		...
		public async Task CreateFile_StreamsCreated()
		{
			...
			var filePath = Path.Combine(this.TempPath, "Created.Txt");
			watcher.Start();
		...
		public async Task DeleteFile_StreamsDeleted()
		{
			...
			var filePath = Path.Combine(this.TempPath, "ToDelete.Txt");
			await File.WriteAllTextAsync(filePath, "foo");
			watcher.Start();
		...
		public async Task RenameFile_StreamsRenamed()
		{
			...
			var originalPath = Path.Combine(this.TempPath, "Changed.Txt");
			await File.WriteAllTextAsync(originalPath, "foo");
			watcher.Start();

			var renamedPath = Path.Combine(this.TempPath, "Renamed.Txt");
